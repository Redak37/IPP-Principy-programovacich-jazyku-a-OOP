...
if (isset($args['parse-only'])) {
    ...
    foreach ($files as $key => $value) {
        echo("<tr bgcolor=#bbb>\n<td bgcolor=");
        exec('python3 '.$args['int-script'].' --source="'.$value.'.src" --input="'.$value.'.in" >'.$dif, $out, $code[0]);
        $num = trim(file_get_contents($value.'.rc'));
        
        if ($num != $code[0]) {
            echo("red>".$value."</td>\n<td></td>\n<td bgcolor=red>".$code[0]."/".$num."</td>\n");
        } elseif ($code[0] == 0) {
            exec('diff "'.$value.'.out" '.$dif, $out, $code[1]);
            if ($code[1] != 0)
                echo("red>".$value."</td>\n<td bgcolor=red>Not same</td>\n<td bgcolor=green>0/0</td>\n");
            else {
                echo("green>".$value."</td>\n<td bgcolor=green>Same</td>\n<td bgcolor=green>0/0</td>\n");
    ...
    foreach ($files as $key => $value) {
        ...
        if ($code[0] != 0) {
            ...
            exec('python3.6 '.$args['int-script'].' --input="'.$value.'.in" --source='.$dif.' > '.$dif.'.log', $out, $code[0]);
            
            if ($num == $code[0]) {
                exec('diff '.$dif.'.log "'.$value.'.out"', $out, $code[0]);
                $cnt++;
                
                if ($code[0] == 0) {
                    echo("green>".$value."</td>\n<td bgcolor=green>Same</td>\n<td bgcolor=green>".$code[0]."/".$code[0]."</td>\n");
                } else
                    echo("green>".$value."</td>\n<td bgcolor=green>Nothing</td>\n<td bgcolor=green>".$num."/".$num."</td>\n");
            } else
                echo("red>".$value."</td>\n<td></td>\n<td bgcolor=red>".$code[0]."/".$num."</td>\n");
        }
